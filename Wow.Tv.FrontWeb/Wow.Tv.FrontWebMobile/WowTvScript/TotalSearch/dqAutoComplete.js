...
function dq_getAcResultList() {
    ...
    if (dq_acuse == 1) {
        if (dq_searchResultList != null && dq_searchResultList.length > 0) {
            ...
            for (var i = 0; i < dq_searchResultList.length; i++) {
                ...
                text += "<li id='dq_ac" + (i + 1) + "' onmouseover=\"dq_setAcPos('" + (i + 1) + "');\" onFocus=\"dq_setAcPos('" + (i + 1) + "');\" onmouseout=\"dq_setAcPos(0);\"  onBlur=\"dq_setAcPos(0);\" onclick=\"goSearch('" + keywordOrign + "');\" onkeypress=\"dq_setAcInput('" + keywordOrign + "');\" style=\"this.style.backgroundColor=''\" >";
    ...
    text += "<p class=\"title\">인기검색어</p><ul>";

    for (var i = 0; i < autoTrendArray.length; i++) {
        text += "<li><a href=\"javascript:goSearch('" + autoTrendArray[i] + "');\">" + autoTrendArray[i] + "</a><span class=\"hits\">" + autoTrendCntArray[i] + "회</span></li>";
    }
    text += "</ul>";
    text += "<p class=\"title\">자동완성";
    if (dq_acuse == 1) {
        text += "<label class=\"switch-btn\" onclick=\"dq_acOff()\">";
        text += "<input class=\"switch-input\" type=\"checkbox\" checked=\"checked\"/>";
    }
    else {
        text += "<label class=\"switch-btn\" onclick=\"dq_acon()\">";
        text += "<input class=\"switch-input\" type=\"checkbox\"/>";
    }
